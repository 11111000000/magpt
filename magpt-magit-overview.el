          ...
          (when data
            (let* ((summary (alist-get 'summary data))
              ...
              (when (stringp summary)
                (dolist (ln (split-string (string-trim-right summary) "\n"))
                  (insert "  " ln "\n")))
              ;; Risks (collapsible; compact-truncated)
              (magit-insert-section (magit-section 'magpt-ai-explain-risks)
                (let ((ic (and (fboundp 'magpt--icon) (magpt--icon 'risk))))
                  (magit-insert-heading (concat (if (and ic (> (length ic) 0)) (concat ic " ") "")
                ...
                (when (and compact (> (length risks) (length rs)))
                  (insert (format "  … %d more\n" (- (length risks) (length rs))))))
              ;; Suggestions (collapsible; multiple, compact-truncated)
              (magit-insert-section (magit-section 'magpt-ai-explain-suggestions)
                (magit-insert-heading (concat "🟢 " (magpt--i18n 'overview-suggestions)))
                (let ((magit-section-initial-visibility-alist
                       (cons (cons 'magpt-ai-suggestion 'show)
  ...
  (when ag
    (magit-insert-section (magit-section 'magpt-ai-card-ask-git)
      (let* ((raw-ic (and (fboundp 'magpt--icon) (magpt--icon 'ask-question)))
             ...
             (q (or (plist-get ag :question) ""))
             (qshort (if (and (stringp q) (> (length q) 0))
                         (let ((s (string-trim q)))
                           (if (> (length s) 80) (concat (substring s 0 80) "…") s))
                       "")))
        (magit-insert-heading
          (concat (if (and ic (> (length ic) 0)) (concat ic " ") "")
                  (if (> (length qshort) 0)
                      (format "%s %s" (magpt--i18n 'overview-card-ask-git-to) qshort)
        ...
        (let ((data (magpt--overview--entry-data ag)))
          (if data
              (let* ((answer (or (alist-get 'answer data)
                     ...
                     (sugs (and (listp (alist-get 'suggestions data))
                                (alist-get 'suggestions data)))
                     (notes (alist-get 'notes data)))
                (when (stringp answer)
                  (dolist (ln (split-string (string-trim-right answer) "\n"))
                    (insert "  " ln "\n")))
                (when (and sugs (> (length sugs) 0))
                  (insert (format "  %s\n" (magpt--i18n 'overview-steps)))
                  (let ((i 1)
                        (magit-section-initial-visibility-alist
                         (cons (cons 'magpt-ai-suggestion 'show)
                               magit-section-initial-visibility-alist)))
                    (dolist (s sugs)
                      (let* ((title (or (alist-get 'title s) (format "Step %d" i)))
                             (keys (or (alist-get 'keys s) (alist-get 'magit_keys s)))
                             (keys-str (and (listp keys)
                                            (string-join (mapcar (lambda (k) (format "%s" k)) keys) ", ")))
                             (cmds (let ((c (alist-get 'commands s)))
                        ...
                        (magit-insert-section (magit-section 'magpt-ai-suggestion (list :index i :data s))
                          ...
                          (when (stringp first-cmd)
                            ...
                            (let ((label (let ((ic (and (fboundp 'magpt--icon) (magpt--icon 'eshell-button))))
                              ...
                              (insert "\n")))))
                      (setq i (1+ i)))))
                (when (and (stringp notes) (> (length (string-trim notes)) 0))
                  (let ((label (condition-case _ (magpt--i18n 'overview-notes) (error "Notes"))))
                    (insert (format "  %s\n" label))
                    (dolist (ln (split-string (string-trim-right notes) "\n"))
                      (insert "    " ln "\n")))))
            (progn
              (insert (magpt--i18n 'overview-response) "\n")
