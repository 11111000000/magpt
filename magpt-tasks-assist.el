...
Be sure to consider all messages, errors, hints and warnings present in the 'RECENT GIT OUTPUT' section below."
  (let* ((ilang (or magpt-info-language "English"))
         ...
         (facts-block
          (let* ((sf (and (listp staged-files) staged-files))
            ...
            (format (concat
                     ...
                     "- If STAGED FILES, UNSTAGED FILES, and UNTRACKED FILES are all empty (0 each), there is nothing to stage or commit; do not suggest 'git add', 'git commit', or 'git push'.\n"
                     "- When RECENT GIT OUTPUT contradicts the FACTS, prefer the FACTS and treat RECENT GIT OUTPUT only as context for errors/hints of ongoing operations (e.g., unfinished rebase), not as an indication of pending file changes.\n\n"
                     "--- BEGIN FACTS ---\n"
                     "STAGED FILES (%d):\n%s\n\n"
                     "UNSTAGED FILES (%d):\n%s\n\n"
                     "UNTRACKED FILES (%d):\n%s\n"
    ...
    (format (concat
             ...
             "Instructions:\n"
             "- Interpret Git status codes (see legend). If there is recent command output with errors/hints/pending steps — analyze it and provide solutions, but DO NOT contradict the FACTS about the current file sets; never infer pending changes when all sets are empty.\n"
